...
int GC::DEFAULT_TRIAL_DURATION = 3000;
...
bool GC::loadConfig(string _filename)
{
    ...
    if(doc.HasMember("DEFAULT_TRIAL_DURATION")) GC::DEFAULT_TRIAL_DURATION = doc["DEFAULT_TRIAL_DURATION"].GetInt();
    else {cout << "NO DEFAULT_TRIAL_DURATION in config.txt, using 3000ms";}
...
{
    if(i2c_fd >= 0) {
        std::cerr << "I2C device already opened fd=" << i2c_fd << "\n";
        return true;
    }
    i2c_fd = open(i2c_device, O_RDWR);
    if(i2c_fd < 0) {
        std::cerr << "Errore apertura I2C: " << std::strerror(errno) << "\n";
        return false;
    }
    std::cerr << "Opened I2C device " << i2c_device << " fd=" << i2c_fd << "\n";
    if(ioctl(i2c_fd, I2C_SLAVE, arduino_address) < 0) {
        std::cerr << "Errore set address I2C: " << std::strerror(errno) << "\n";
        close(i2c_fd);
        i2c_fd = -1;
        return false;
    }
...
void GC::closeI2C()
{
    if(i2c_fd >= 0) {
        close(i2c_fd);
        std::cerr << "Closed I2C device fd=" << i2c_fd << "\n";
        i2c_fd = -1;
    }
}
...
bool GC::writeMessageArduino(string &data)
{
    // ensure the I2C file descriptor is valid
    if(i2c_fd < 0) {
        std::cerr << "I2C device not opened (i2c_fd < 0)\n";
        return false;
    }

    // Dump bytes to send (hex) for diagnostics
    std::cerr << "I2C write: length=" << data.size() << " bytes: ";
    ...
    for(int attempt=1; attempt<=MAX_RETRIES; ++attempt) {
        ssize_t bytes_written = write(i2c_fd, data.c_str(), data.size());
        if(bytes_written < 0) {
            int e = errno;
            std::cerr << "Attempt " << attempt << ": Error writing to I2C device: " << std::strerror(e) << " (" << e << ")\n";

            // If EIO (no ACK / bus error), try to recover: close and reopen and then ioctl fallback
            if(e == EIO) {
                std::cerr << "EIO detected: closing and reopening I2C device, then attempting ioctl fallback\n";
                closeI2C();
                usleep(50000); // 50 ms
                if(!initI2C()) {
                    std::cerr << "Re-open I2C failed during recovery\n";
                    // wait before next attempt
                    usleep(100000);
                    continue;
                }

                // try ioctl I2C_RDWR as fallback
                std::vector<uint8_t> buf(data.begin(), data.end());
                struct i2c_msg msg;
                msg.addr = arduino_address;
                msg.flags = 0;
                ...
                if(ioctl(i2c_fd, I2C_RDWR, &ioctl_data) < 0) {
                    std::cerr << "I2C_RDWR ioctl attempt failed: " << std::strerror(errno) << "\n";
                    // try again in next loop iteration
                    usleep(100000);
                    continue;
                } else {
                    std::cerr << "I2C_RDWR ioctl succeeded on attempt " << attempt << "\n";
                    return true;
                }
            }

            // non-recoverable error for this attempt; short delay then retry
            usleep(50000);
            continue;
        }
        if(static_cast<size_t>(bytes_written) != data.size()) {
            std::cerr << "Partial write to I2C device: wrote " << bytes_written << " of " << data.size() << " bytes\n";
    ...
    std::cerr << "All write attempts failed\n";
    return false;
}
